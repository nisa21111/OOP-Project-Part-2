...
Robot* createRobotFromFile(std::istream& in){
    ...
    if (!r)
        throw std::runtime_error("Filed to create robot from file");
    r->loadFromFile(in);
    return r;
}
...
Weapon* createWeaponFromFile(std::istream& in){
    ...
    if (!w)
        throw std::runtime_error("Failed to create weapon from file");
    w->loadFromFile(in);
    return w;
}
